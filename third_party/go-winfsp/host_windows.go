...
// FileInfoTimeout sets FSP_FSCTL_VOLUME_PARAMS.FileInfoTimeout (milliseconds).
// 0 disables FSD metadata caching; ^uint32(0) enables infinite metadata caching
// and the NTOS Cache Manager for file data.
//
// ClassicStack patch: upstream go-winfsp v1.0.3 does not expose this field.
func FileInfoTimeout(ms uint32) Option {
	return func(o *option) {
		o.fileInfoTimeout = ms
	}
}
