...
func hostSetxattr(path0 *c_char, name0 *c_char, buff0 *c_char, size0 c_size_t,
	...
	buff := (*[1 << 30]byte)(unsafe.Pointer(buff0))
	if xp, ok := fsop.(FileSystemXattrP); ok {
		return c_int(xp.SetxattrP(path, name, buff[:size0], int(flags), position))
	}
	return c_int(fsop.Setxattr(path, name, buff[:size0], int(flags)))
}

func hostGetxattr(path0 *c_char, name0 *c_char, buff0 *c_char, size0 c_size_t,
