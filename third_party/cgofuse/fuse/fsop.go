...
// FileSystemXattrP is the interface that wraps Darwin-position xattr methods.
//
// macOS getxattr/setxattr take an extra position argument used only for the
// resource-fork attribute (com.apple.ResourceFork): it is a byte offset into
// the fork so Finder/cp/ditto can read and write large forks in chunks. The
