...
- **Deferred to its own slice (NOT this one): the wire push.** The notify sink is currently a no-op counter — it does NOT yet emit AFP attention or SMB CHANGE_NOTIFY frames. Reason: SMB's session seam (`conn.go` `ServeMessage(req)→reply`) is strictly request→response with no server-initiated channel, and classic AFP has no per-directory change-notify (only volume-mod-date polling + a heavy server-message attention). Emitting real frames needs a new async-push contract across the four SMB transports (NBF/NBIPX/NBT/direct) + the AFP attention path. The §10d coordination plumbing (publish → shared bus → Origin-filtered, share-attributed delivery) is complete and tested end-to-end; turning the resolved notification into wire frames is the follow-on.
