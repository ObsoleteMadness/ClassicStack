...
| M8a | Share config + Manager wiring: AFP/SMB volume `core/config` sections + `config → []fs.ShareSpec` mapper (options→Extra) in the registry factories; supervisor `Reconfigure` for an AFP/SMB section drives `share.Manager` Add/Update/Remove; `ParamsFor`-generated per-fs_type form masks `secret` params (§9d/§11). **Plus server identity (§4-bis):** top-level `config.Identity{Hostname,Workgroup}` (one source of truth, NOT per-service); registry hands one `Hostname` to NetBIOS + SMB (`SetServerName`, advertised in NEGOTIATE) + browser; no per-service hostname field so they cannot diverge; `Validate` backstops any externally-surfaced second name; `Hostname` change is restart-grade for NetBIOS. | M7c,M8 | | ⬜ |
