...
//
// A prior implementation tried to honor MPX per [MS-CIFS] 2.2.4.23 by
// returning a single WCT=8 response with the full chunk. On the wire
// (see captures/ipx.pcap frames 365–393 and 415+) Win9x silently
// rejected those responses and retransmitted the same request at
// offset 0 forever — exact root cause unknown, but Samba avoids the
// command for the same reason. See spec/errata.md.
