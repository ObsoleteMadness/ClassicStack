...
# gosec scans only the packages that handle untrusted external input: the MacIP
# gateway service, its DHCP/NAT adapter (which parses DHCP options and forwards
# real TCP/IP off-net), and macgarden (which fetches and parses web content).
GOSEC_PKGS=(
  ./core/service/macip/...
  ./adapter/macipgw/...
  ./adapter/macgarden/...
