 ...
 * readable. Portable format — see tools/end-to-end/RESULT-FORMAT.md for the
 * cross-OS spec every platform's tool must emit. File I/O itself goes through
 * a platform seam so this header stays platform-agnostic. Kept identical to
 * the macOS tool's results.h.
 */
 ...
 * timestamp string (see spec: "RESULT v1 started=..."), already quoted. */
 ...
 * platform's diagnostic console isn't available/visible — results.txt is
 * the one output channel these tools always reliably produce. */
