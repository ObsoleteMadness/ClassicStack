...
func appConfigFromModel(m *config.Model) (appConfig, error) {
	...
	// defines its own [<Section>.Custom] interface. The scalar `interface`
...
// string (the legacy `<Section>.interface` key) still overrides the device for
...
// customIfDiffers reconstructs a protocol's [<Section>.Custom] interface for
