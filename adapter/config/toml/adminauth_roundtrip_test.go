...
// TestAdminAuthRoundTrip proves the §4-ter web-admin credential survives a TOML
// round-trip: the username and salted hash a /setup writes are read back verbatim,
// and the reloaded credential still verifies the original password.
...
// [adminauth] block, so a fresh server.toml stays in first-run state on reload.
