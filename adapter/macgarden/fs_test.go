...
func TestMacGardenChildCount_CategoryIsLazyUntilCached(t *testing.T) {
	...
	fsys := &MacGardenFileSystem{
		root:              root,
		categories:        []Category{{Name: "Antivirus", URL: "https://macintoshgarden.org/apps/utilities/antivirus"}},
...
func TestMacGardenReadDirRange_UsesCachedFirstAndLastPages(t *testing.T) {
	...
	fsys := &MacGardenFileSystem{
		root:              root,
		categories:        []Category{{Name: "Antivirus", URL: catURL}},
		categoryItemCount: make(map[string]uint16),
...
func TestMacGardenReadDirRange_CategoryReqCountIsCappedToFirstWindow(t *testing.T) {
	root := filepath.Clean(t.TempDir())
	catURL := "https://macintoshgarden.org/apps/utilities/antivirus"
	firstPage := make([]SearchResult, 0, 10)
	for i := 1; i <= 10; i++ {
		firstPage = append(firstPage, SearchResult{
...
func TestMacGardenStat_ItemChildIsLazyUntilItemOpened(t *testing.T) {
	...
	fsys := &MacGardenFileSystem{
		root:         root,
		categories:   []Category{{Name: "3D Rendering & CAD", URL: catURL}},
	...
	if err == nil {
		t.Fatal("expected iofs.ErrNotExist for unopened item child path")
	}
	if !errors.Is(err, iofs.ErrNotExist) {
		t.Fatalf("Stat error = %v, want %v", err, iofs.ErrNotExist)
	}
		...
		itemByURL: map[string]*SoftwareItem{
			itemURL: {
				...
				Downloads: []DownloadDetails{{
					Title: "Alias upFRONT 2.0",
					Links: []DownloadLink{{Text: "Download", URL: "://bad-download-url"}},
				}},
			},
		},
...
func TestMacGardenCatSearch_UsesTypeSubdirectoryWhenKnown(t *testing.T) {
	...
	// New-ring CatSearcher contract: free-text query in CatSearchCriteria, an opaque
	// resumption cursor, and []CatSearchResult with STORE-relative paths back. No AFP
	// protocol types leak through the FileSystem seam.
	results, _, err := fsys.CatSearch(corefs.CatSearchCriteria{Query: query, Max: 10}, nil)
	if err != nil {
		t.Fatalf("CatSearch error: %v", err)
	}
	if len(results) != 2 {
		t.Fatalf("len(results)=%d, want 2", len(results))
	}
	// Paths are volume-relative ('/'-separated), not joined with the host root.
	if results[0].Path != "search/"+query+"/App/ClarisWorks 4.0" {
		t.Fatalf("results[0].Path=%q, want typed store path", results[0].Path)
	}
	if results[1].Path != "search/"+query+"/Mystery Result" {
		t.Fatalf("results[1].Path=%q, want untyped store path", results[1].Path)
	}
	// Each match is materialised as a virtual directory.
	if results[0].Info == nil || !results[0].Info.IsDir() {
		t.Fatalf("results[0].Info not a dir: %#v", results[0].Info)
	}
}
