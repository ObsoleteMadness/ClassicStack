// Package nat provides the host-network NAT engine and IPv4 packet utilities used
// by the MacIP gateway adapter (adapter/macipgw). It forwards off-subnet Mac IP
// traffic through the host OS network stack so the host's own IP is the NAT source,
// avoiding the routing problem that occurs when the MacIP subnet differs from the
// physical network.
//
// Ring: ADAPTER. Unlike core/, this freely uses the net package and golang.org/x/net
// (real OS sockets). Ported from the legacy port/nat (which routed replies straight
// into the AppleTalk router); here the engine instead emits reassembled IPv4 packets
// to a sink callback (the egress forwards them to the MacIP core, which routes them to
// the owning Mac client). This keeps the NAT engine free of any AppleTalk dependency.
package nat

import "encoding/binary"

// MaxIPPerDDP is the maximum IP payload that fits in a single DDP packet
// (ddp.MaxDataLength = 586 bytes). Kept here so the egress can fragment NAT output to
// the DDP MTU before handing it to the core.
