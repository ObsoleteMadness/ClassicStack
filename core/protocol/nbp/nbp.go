// Package nbp holds the AppleTalk Name Binding Protocol codec: function codes,
// tuple layout, packet parser/builder, and the small matching primitives used
// by lookup. It contains no I/O or service state — the registry and routing
// logic that uses these types lives in the service ring.
//
...
// NBP control function codes, carried in the high nibble of the first byte of
// an NBP packet. The low nibble carries the tuple count.
const (
...
// Tuple is a single NBP tuple: an address (network/node/socket), an enumerator,
// and an entity name (object:type@zone). Inbound packets carry exactly one
// tuple in ClassicStack's NBP handler; LkUp-Rply may pack several, but the
// registered service emits one per match.
type Tuple struct {
	Network    uint16
	Node       uint8
	Socket     uint8
...
// payload. It returns ErrMalformed if the layout is invalid or the declared
// lengths run past the buffer.
//
// On-wire layout:
//
//	0       1            2..3       4    5    6    7
...
//
// A trailing zero zone-length is treated as the zone wildcard "*".
...
// BuildLkUpRply encodes a single-tuple LkUp-Rply packet. The returned slice is
// freshly allocated.
...
// ZoneMatch reports whether the given pattern matches the registered zone. NBP
// uses '*' as the zone wildcard.
